...
```{r include = FALSE}
demand <- lm(sales$Demand ~ sales$Price + sales$`Average Industry Price` + sales$`Advertising Expenditure`)
demand$coefficients
```
$$ \beta_0 = demand$coefficients[1], \beta_1 = demand$coefficients[2] $$
```
```
