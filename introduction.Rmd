...
* sum of all forecast errors over time, $$\sum_{t=1}^{n}{(y_t - \hat{y}_t})$$, has been proposed but is not used because in case errors show random pattern they will cancel each other and the sum of the forecast error will be near zero. 
* mean absolute deviation (MAD), $$\frac{\sum_{t=1}^{n}{|e_t|}}{n}$$, can be used to determine the magnitude of the forecast errors generated by the forecasting methodology being used. 
* mean squared error (MSE), $$\frac{\sum{(e_t)^2}}{n}$$ has been also proposed as a way to remedy the issue found with the sum of all forecast errors over time. In contrary to MAD, MSE metric penalises the forecasting method much more for large forecasting errors than for small errors. 
